...
## Demo
run iOS ```cd example && yarn ios``` <br>
...
Prop | Description
---- | -----------
`onStateChange` | Called when media starts playing returns eg { `isPlaying:true`, <br />`position:0.1`, <br />`currentTime:0.1`, <br />`duration: 0.1`, <br />`bufferRate?: 1.0`, <br />`type: {string}`}
`onLayoutChange` | Called when the layout change returns eg {`videoWidth:0`, `videoHeight:0`, `videoVisibleWidth:0`, `videoVisibleHeight:0`, `sarNum:0.1`, `sarDen:0.1`}
